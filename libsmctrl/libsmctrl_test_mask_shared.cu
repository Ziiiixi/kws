...
int test_constrained_size_and_location(enum partitioning_type part_type) {
  ...
  const int CONTEXT_POOL_SIZE = 1;
  CUcontext contextPool[CONTEXT_POOL_SIZE];
  int smCounts[CONTEXT_POOL_SIZE] = {num_sms};
  
  CUexecAffinityParam affinity;
  affinity.type = CU_EXEC_AFFINITY_TYPE_SM_COUNT;
  affinity.param.smCount.val = smCounts[0];
  cuCtxCreate_v3(&contextPool[0], &affinity, 1, 0, 0);
  cuCtxSetCurrent(contextPool[0]);

  int numSms = smCounts[0];
  int numBlocksPerSm = 0;
  int numThreads = 1024;

  cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, read_and_store_smid, numThreads, 0);

  int totalThreads = numSms * numBlocksPerSm * numThreads;
  ...
  printf("num_tpcs: %d", num_tpcs);

  // printf("num_sms %d, num_tpcs %d, sms_per_tpc: %d",num_sms, num_tpcs, sms_per_tpc);
  ...
  // read_and_store_smid<<<dimGrid, dimBlock, 0, stream>>>(smids_native_d);
  ...
  //     // printf("smids_partitioned_h: %s", uniq_partitioned);
  //     printf("%s: ***Test failure.***\n"
